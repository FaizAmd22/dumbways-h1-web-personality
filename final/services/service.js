...
async function projects(req, res) {
    ...
    let authorFilter = req.query.authorFilter 
    let projectNameFilter = req.query.projectNameFilter 

    console.log("author : ", authorFilter)
    console.log("title : ", projectNameFilter)

    let query = `SELECT tb_projects.id, tb_projects.name, tb_projects.start_date, tb_projects.end_date, tb_projects.description, tb_projects.node, tb_projects.next, tb_projects.react, tb_projects.typescript, tb_projects.image, tb_projects.duration, 
    ...
    if (whereClauses.length > 0) {
        ...
        query += ' ORDER BY tb_projects.id';
        const object = await sequelize.query(query, { type: QueryTypes.SELECT })
    
        res.render('project', { datas: object.reverse(), isLogin, user, authorFilter, projectNameFilter })
    } else {
        query += ' ORDER BY tb_projects.id';
        const object = await sequelize.query(query, { type: QueryTypes.SELECT })
